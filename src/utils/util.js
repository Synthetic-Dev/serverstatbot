...
if (!String.prototype.toTitleCase) {
    String.prototype.toTitleCase = function (allWords = false) {
        ...
        if (!allWords)
            return (
                this.toTitleCase(string.shift(), true) + " " + string.join(" ")
            )
...
class Util {
    ...
    static error(e, file, scope) {
        console.error(
            `${file}${scope ? `[${scope}]` : ""}: ${
                e.method || e.path
                    ? `${e.method ? `${e.method}` : "?"} at ${
                          e.path ? `${e.path}` : "?"
                      }\n`
                    : ""
            }\u001b[1;31m${e.stack ? `${e.stack}` : "Missing stack"}`
        )
    }
    ...
    static request(options, callback) {
        ...
        const method = isUrl
            ? options.toLowerCase().slice(0, 5) == "https"
                ? HTTPS
                : HTTP
    ...
    static loadmodules(dir, register) {
        ...
        FileSystem.readdir(`${__dirname}/../${dir}`, (error, files) => {
            ...
            if (jsfiles.length === 0)
                return console.log(`No .js files to load at /${dir}`)
    ...
    static decorateText(context, text, style, x, y, thickness) {
        ...
        thickness = context.font.toLowerCase().includes("bold")
            ? thickness * 1.5
            : thickness
        ...
        if (style.includes("strikethrough")) {
            context.fillRect(
                x,
                y + metrics.actualBoundingBoxDescent - height / 2,
                width,
                thickness * 1.2
            )
        }
    ...
    static autoCropCanvas(context, padding = 0) {
        ...
        let cropped = context.getImageData(
            Math.max(0, pixels.x[0] - padding),
            Math.max(0, pixels.y[0] - padding),
            w,
            h
        )
    ...
    static createGraph(graphData, xAxis, yAxis) {
        ...
        graphData.forEach(({ x, y }, index) => {
            let remove = false
            if (
                (xAxis.start && x < xAxis.start) ||
                (yAxis.start && y < yAxis.start)
            )
                remove = true
            ...
            if (!remove && !xAxis.start)
                smallestX = smallestX == null || x < smallestX ? x : smallestX
            if (!remove && !yAxis.start)
                smallestY = smallestY == null || y < smallestY ? y : smallestY
        ...
        if (xAxis.clamp) {
            xAxis.start =
                Math.floor(smallestX / xAxis.increment) * xAxis.increment
        ...
        if (yAxis.clamp) {
            yAxis.start =
                Math.floor(smallestY / yAxis.increment) * yAxis.increment
        ...
        context.lineTo(
            graphWidth - totalPadding + numberPadding / 2,
            graphHeight - totalPadding
        )
        context.stroke()
        context.closePath()
        ...
        context.fillText(
            xAxis.name,
            graphWidth / 2,
            graphHeight - labelPadding / 2 + fontSize / 2,
            graphWidth - labelPadding * 2
        )
        ...
        context.fillText(
            yAxis.name,
            -graphHeight / 2,
            labelPadding / 2 - graphWidth,
            graphHeight - labelPadding * 2
        )
        context.restore()
        ...
        const maxIndexX = Math.round(
            (xAxis.max - xAxis.start) / xAxis.increment
        )
        const maxIndexY = Math.round(
            (yAxis.max - yAxis.start) / yAxis.increment
        )
        ...
        for (
            let x = xAxis.start;
            x < xAxis.max + xAxis.increment / 2.1;
            x += xAxis.increment
        ) {
            ...
            context.lineTo(
                x1,
                -(graphHeight - totalPadding * 2 + numberPadding / 2)
            )
        }

        for (
            let y = yAxis.start;
            y < yAxis.max + yAxis.increment / 2.1;
            y += yAxis.increment
        ) {
            ...
            context.moveTo(0, y1)
            context.lineTo(
                graphWidth - totalPadding * 2 + numberPadding / 2,
                y1
            )
        }
        context.stroke()
        ...
        for (
            ...
            let w =
                (graphWidth - totalPadding * 2) * percentage +
                context.lineWidth / 2
            let x1 = w / ca
            let y1 = x1 * ta
        ...
        for (
            ...
            context.fillText(
                value,
                -context.lineWidth - 3,
                -(graphHeight - totalPadding * 2) * percentage +
                    fontSize / 2 -
                    context.lineWidth / 2,
                numberPadding * 1.2
            )
    ...
    static dmUser(user, content) {
        return new Promise((resolve, reject) => {
            user.createDM()
                .then((dmChannel) => {
                    this.sendMessage(dmChannel, content)
                        .then(resolve)
                        .catch(reject)
                })
                .catch(reject)
    ...
    static replyMessage(message, content) {
        return new Promise((resolve, reject) => {
            ...
            if (channel instanceof Discord.DMChannel) {
                ...
                let hasPerms = this.hasPermissionsInChannel(
                    message.guild.me,
                    channel,
                    ["SEND_MESSAGES"]
                )
    ...
    static replyWarning(message, warning) {
        this.replyMessage(message, ":warning: " + warning)
            .then((botMessage) => {
                if (
                    !botMessage ||
                ...
                botMessage.client.setTimeout(() => {
                    botMessage.delete().catch((e) => {
                        this.error(e, "Util", "deleteMessage1")
                    })
                }, 15000)
            })
            .catch((e) => {
                this.error(e, "Util", "replyMessage1")
            })
    }
    ...
    static sendWarning(medium, warning) {
        this.sendMessage(medium, ":warning: " + warning)
            .then((botMessage) => {
                if (
                    !botMessage ||
                    botMessage.channel instanceof Discord.DMChannel
                ...
                botMessage.client.setTimeout(() => {
                    botMessage.delete().catch((e) => {
                        this.error(e, "Util", "deleteMessage2")
                    })
                }, 15000)
            })
            .catch((e) => {
                this.error(e, "Util", "sendMessage1")
            })
    }
    ...
    static replyError(message, error) {
        this.replyMessage(message, ":stop_sign: " + error)
            .then((botMessage) => {
                if (
                    !botMessage ||
                    botMessage.channel instanceof Discord.DMChannel
                ...
                botMessage.client.setTimeout(() => {
                    botMessage.delete().catch((e) => {
                        this.error(e, "Util", "deleteMessage3")
                    })
                }, 15000)
            })
            .catch((e) => {
                this.error(e, "Util", "replyMessage2")
            })
    }
    ...
    static sendError(medium, error) {
        this.sendMessage(medium, ":stop_sign: " + error)
            .then((botMessage) => {
                if (
                    !botMessage ||
                    botMessage.channel instanceof Discord.DMChannel
                ...
                botMessage.client.setTimeout(() => {
                    botMessage.delete().catch((e) => {
                        this.error(e, "Util", "deleteMessage4")
                    })
                }, 15000)
            })
            .catch((e) => {
                this.error(e, "Util", "sendMessage2")
            })
    }
    ...
    static couldNotFind(message, type, input, inObject = null) {
        const locale = message.guild
            ? message.guild.preferredLocale ?? "en-us"
            : "en-us"
        const lang = LocaleManager.getLang(locale)
        this.replyError(
            message,
            inObject
                ? lang.COULDNOTFIND.format(type.toLowerCase(), input)
                : lang.COULDNOTFIND_IN.format(
                      type.toLowerCase(),
                      input,
                      inObject
                  )
     ...
     * @param {{channel: Discord.TextChannel, author: Discord.User} | Discord.Message} options
    ...
    static sendPages(
        ...
        console.assert(
            timeout || maxTime,
            "'timeout' and 'maxTime' cannot both be null."
        )

        if (options instanceof Discord.Message) {
            options = { channel: options.channel, author: options.author }
        }

        const locale = options.channel.guild
            ? options.channel.guild.preferredLocale ?? "en-us"
            : "en-us"
        const lang = LocaleManager.getLang(locale)
            ...
            pages.forEach((page, index) => {
                if (page.embed) {
                    ...
                    page.embed.footer.text =
                        page.embed.footer.text ?? options.author.tag
                    page.embed.footer.icon_url = options.author.avatarURL({
                        size: 32,
                        dynamic: true,
                        format: "png",
                    })
                ...
                page.component = page.component ?? new MessageActionRow()

                let pageNum = new MessageButton()
                    .setStyle("blurple")
                    .setLabel(lang.PAGE_NUM.format(index + 1, pages.length))
                    .setID("pages_label")
                ...
                page.component.addComponent(pageNum)
                if (start) page.component.addComponent(start)
                page.component.addComponents(back, next)
                if (end) page.component.addComponent(end)
            })

        let pageIndex = startPage
        this.sendMessage(options.channel, pages[pageIndex])
            .then((botMessage) => {
                if (
                    !botMessage ||
                    pages.length == 1 ||
                ...
                let collector = botMessage.createButtonCollector(
                    (button) => button.clicker.user.id == options.author.id,
                    {
                        time: maxTime * 1000,
                        idle: timeout * 1000,
                        dispose: true,
                ...
                collector.on("collect", (button) => {
                    ...
                    switch (button.id) {
                        ...
                        case "pages_next":
                            pageIndex =
                                pageIndex + 1 < pages.length ? pageIndex + 1 : 0
                            break
                        case "pages_start":
                            pageIndex = 0
                            break
                    ...
                    botMessage.edit(pages[pageIndex]).catch((e) => {
                        this.error(e, "Util", "editMessage1")
                    })
                })
                ...
                collector.on("end", async () => {
                    ...
                    page.component.components.forEach((button) => {
                        if (button.custom_id == "pages_label") {
                            button.setLabel("Timeout")
                        }
                        button.setDisabled(true)
                    })

                    botMessage.edit(page).catch((e) => {
                        this.error(e, "Util", "editMessage2")
                    })
                })
            })
            .catch((e) => {
                this.error(e, "Util", "sendMessage3")
            })
    }
        ...
        try {
            ...
            permissions.forEach((permission) => {
                ...
                if (permission == "DEV") {
                    ...
                    flag = member.hasPermission(permission, {
                        checkAdmin: true,
                        checkOwner: true,
                    })
                }
    ...
    static isOnMobile(user) {
        ...
        return (
            status.mobile &&
            (!status.desktop || status.desktop == "dnd") &&
            (!status.web || status.web == "dnd")
        )
    }
    ...
    static getEmoji(client, input) {
        ...
        client.emojis.cache.forEach((emoji) => {
            if (
                !find &&
                (emoji.name == input || emoji.name == unicodeEmojis[input])
            )
                find = emoji
        })
    ...
    static areEmojisEqual(...emojis) {
        ...
        return (
            emojis.filter((emoji) => emoji == emojis[0]).length == emojis.length
        )
    }
    ...
    static getPriorityChannel(guild, check) {
        try {
            let channel = this.getChannelById(
                guild.channels,
            ...
            attemptNames.forEach((name) => {
                ...
                if (
                    channel &&
                    (!channel.viewable || (check && !check(channel)))
                )
                    channel = null
            })
            return channel
        ...
        return new Promise(async (resolve, reject) => {
            ...
            while (messages.length < count) {
                ...
                msgs.each((message) => {
                    ...
                    if (
                        !leastMessage ||
                        message.createdTimestamp < leastMessage.createdTimestamp
                    ) {
    ...
    static getFooter(message, hasUptime = true) {
        ...
        return {
            text: `${
                hasUptime
                    ? `${lang.UPTIME}: ${lang.TIME_FORMAT.format(
                          Math.floor(client.uptime / 1000 / 3600),
                          Math.floor((client.uptime / 1000 / 60) % 60),
                          Math.floor((client.uptime / 1000) % 60)
                      )} | `
                    : ""
            }${lang.COPYRIGHT.format(new Date().getUTCFullYear())} (${
    ...
    static async parseRole(guild, input) {
        input = input.replace(/^<@&/, "").replace(/>$/, "")
        return new Promise((resolve, reject) => {
            this.getRoleById(guild, input)
                .then((find) => {
                    if (find) return resolve(find)
                    this.getRole(guild, input)
                        .then((find) => {
                            return resolve(find)
                        })
                        .catch(reject)
    ...
    static parseDate(input, lang) {
        ...
        if (!date) {
            ...
            let namedDates = {
                ...
                [lang[TIME_KEYWORD_TODAY]]: new Date(
                    currentDate.getUTCFullYear(),
                    currentDate.getUTCMonth(),
                    currentDate.getUTCDate()
                ),
                [lang[TIME_KEYWORD_YESTERDAY]]: new Date(
                    currentDate.getUTCFullYear(),
                    currentDate.getUTCMonth(),
                    currentDate.getUTCDate() - 1
                ),
                [lang[TIME_KEYWORD_THISWEEK]]: new Date(
                    currentDate.getUTCFullYear(),
                    currentDate.getUTCMonth(),
                    currentDate.getUTCDate() - currentDate.getUTCDay()
                ),
                [lang[TIME_KEYWORD_LASTWEEK]]: new Date(
                    currentDate.getUTCFullYear(),
                    currentDate.getUTCMonth(),
                    currentDate.getUTCDate() - currentDate.getUTCDay() - 7
                ),
            }
