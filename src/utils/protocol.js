...
class Protocol {
    ...
    static getErrorType(error) {
        ...
        if (
            failStrings.includes(error.message) ||
    ...
    static isIpValid(ip) {
        const validIp_HostnameRegex =
            /^(((([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5]))|((([a-zA-Z]|[a-zA-Z0-9][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)+([A-Za-z]|[A-Za-z][A-Za-z0-9\‌​-]*[A-Za-z0-9])))$/g
    ...
    static async sendRequest(ip, port = 25565, queryPort = port) {
        if (this.blockedHosts.includes(ip.toLowerCase()))
            return [false, new Error("Blocked host")]
        if (port > this.maxPort)
            return [false, new Error("Port exceeds maximum")]
        if (queryPort > this.maxPort)
            return [false, new Error("Query port exceeds maximum")]
        ...
        let bulkData = new Promise((resolve) => {
            ...
            function query(statusResponse) {
                if (statusResponse)
                    statusResponse.bedrock = !!statusResponse.bedrock
                    ...
                    .then((queryResponse) => {
                        ...
                        if (statusResponse) {
                            Object.keys(statusResponse).forEach((key) => {
                                ...
                                if (
                                    value instanceof MinecraftUtil.Description
                                ) {
                                    if (
                                        queryResponse[key].descriptionText == ""
                                    ) {
                                        queryResponse[key] = value
                                    }
                                }
                            ...
                            queryResponse.bedrock =
                                queryResponse.edition == "MCPE" ||
                                queryResponse.serverGUID != null
                        }
        ...
        let forgeData = result?.rawResponse?.forgeData
        if (forgeData) {
            if (!result.modInfo) {
                result.modInfo = forgeData
                result.modInfo.type = "Forge"

                if (result.modInfo.mods) {
                    result.modInfo.modList = result.modInfo.mods
                    delete result.modInfo.mods
                }
            } else if (!result.modInfo.modList) {
                result.modInfo.modList = forgeData.mods
            }
        }
    ...
    static async getInfo(ip, port = 25565, queryPort = port) {
        ...
        if (success) {
            ...
            players.sort((a, b) =>
                a.name.clean > b.name.clean
                    ? 1
                    : b.name.clean > a.name.clean
                    ? -1
                    : 0
            )
            ...
            final = {
                ...
                modded:
                    result.modInfo != null &&
                    result.modInfo.modList != null &&
                    result.modInfo.modList.length > 0,
            ...
            if (final.mods) {
                final.mods.modList.forEach((mod) => {
                    ...
                    if (/^OHNOES/g.test(mod.version)) {
                        mod.version = "unknown"
                    }
                })

                final.mods.modList = final.mods.modList.filter(
                    (mod) =>
                        !["minecraft", "fml", "mcp", "forge"].includes(
