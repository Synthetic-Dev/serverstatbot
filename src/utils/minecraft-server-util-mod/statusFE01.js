...
const formatResultFE01FA_1 = __importDefault(require("minecraft-server-util/dist/util/formatResultFE01FA"));
    ...
    return new Promise(async (resolve, reject) => {   //__awaiter(this, void 0, void 0, function* () {
        ...
        try {
...

            {
                ...
                if (packetType !== 0xFF) reject(new Error('Packet returned from server was unexpected type'));
                ...
                if (isNaN(protocolVersion)) reject(new Error('Server returned an invalid protocol version: ' + protocolVersionStr));
                if (isNaN(playerCount)) reject(new Error('Server returned an invalid player count: ' + playerCountStr));
                if (isNaN(maxPlayers)) reject(new Error('Server returned an invalid max player count: ' + maxPlayersStr));
            }

            // Convert the data from raw Minecraft status payload format into a more human readable format and resolve the promise
            resolve(formatResultFE01FA_1.default(host, opts.port, srvRecord, protocolVersion, serverVersion, motd, playerCount, maxPlayers, Date.now() - startTime));
        }
