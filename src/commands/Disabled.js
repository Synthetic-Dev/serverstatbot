...
class Command extends CommandBase {
    constructor(client) {
        ...
        this.setOptionFunc("add", (options, input) => {
            ...
            if (!command)
                return Util.couldNotFind(options.message, "command", input)
            if (!command.hasTag("CAN_DISABLE"))
                return Util.replyWarning(
                    options.message,
                    options.lang.COMMAND_DISABLED_CANNOT_DISABLE
                )
            ...
            options.settings.update("disabledCommands", (data) => {
                if (data.Commands.includes(command.name)) {
                    Util.replyWarning(
                        options.message,
                        options.lang.COMMAND_DISABLED_ALREADY_DISABLED
                    )
                } else {
                    data.Commands.push(command.name)
                    Util.sendMessage(
                        options.message,
                        options.lang.COMMAND_DISABLED_DISABLED.format(
                            command.name
                        )
                    )
                }
        ...
        this.setOptionFunc("remove", (options, input) => {
            ...
            options.settings.update("disabledCommands", (data) => {
                if (!data.Commands.includes(command.name)) {
                    Util.replyWarning(
                        options.message,
                        options.lang.COMMAND_DISABLED_ALREADY_ENABLED
                    )
                } else {
                    ...
                    Util.sendMessage(
                        options.message,
                        options.lang.COMMAND_DISABLED_ENABLED.format(
                            command.name
                        )
                    )
                }
        ...
        this.setOptionFunc("removeall", (options) => {
            ...
            Util.sendMessage(
                options.message,
                options.lang.COMMAND_DISABLED_ENABLE_ALL
            )
        })
    }
