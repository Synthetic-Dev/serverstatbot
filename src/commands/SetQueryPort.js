...
class Command extends CommandBase {
    constructor(client) {
        super(client, {
            ...
            aliases: ["setqport", "queryport", "qport"],
            args: [
                {
                    name: "port",
    ...
    async execute(options) {
        ...
        if (port == 0 || port == -1) {
            ...
            return Util.replyMessage(
                options.message,
                options.lang.COMMAND_SETQUERYPORT_REMOVED
            ).catch((e) => {
                Util.error(e, "SetQueryPort", "replyMessage1")
            })
        }
        ...
        Util.replyMessage(
            options.message,
            options.lang.COMMAND_SETQUERYPORT_CONTENT.format(port)
        ).catch((e) => {
            Util.error(e, "SetQueryPort", "replyMessage2")
        })
    }
}
