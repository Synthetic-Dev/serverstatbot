...
/**
 * Manages the automatic status messages for a guild
 * @class
 * @extends {Types.Base}
 */
class StatusManager extends Base {
    ...
    playerMessage(player, textFormat) {
        ...
        Canvas.loadImage(`https://mc-heads.net/avatar/${player}/100`)
            .then((head) => {
                ...
                Util.sendMessage(this.channel, {
                    ...
                    Util.error(e, "Logging", "sendMessage1")
                })
            })
            .catch((error) => {
                Util.sendMessage(
                    this.channel,
                    this.lang.STATUS_PLAYER_FAILEDIMAGE.format(text)
                ).catch((e) => {
                    Util.error(e, "Logging", "sendMessage2")
                })
            })
    }
    ...
    async updateChannel() {
        ...
        this.channel = Util.getChannelById(
            this.guild.channels,
        ...
        this.channelChanged =
            ...
            (previousChannel && !this.channel) ||
            (previousChannel && previousChannel.id != this.channel.id)
        if (this.hasChannel) {
            this.canSend = Util.hasPermissionsInChannel(
                this.guild.me,
                this.channel,
                ["SEND_MESSAGES"]
            )
    ...
    async logs(data) {
        if (data.online) {
            if (
                (!this.server.online && !this.statusMessage.online) ||
                this.channelChanged
            ) {
                if (
                    !this.server.start &&
                    this.statusMessage.message &&
                    this.statusMessage.message.member == this.guild.me &&
                    Date.now() - this.statusMessage.message.createdTimestamp <
                        120 * 1000
                ) {
                    this.statusMessage.message
                        .edit(
                            "<:cyan_circle_with_circular_arrow:845041391247032350> " +
                                this.lang[statusContents.restart]
                        )
                        .catch((e) => {
                            Util.error(e, "Logging", "editMessage")
                        })
                } else if (!this.statusMessage.message) {
                    Util.sendMessage(
                        this.channel,
                        "<:green_circle_with_tick:818512512500105249> " +
                            this.lang[statusContents.online]
                    )
                        .then((message) => {
                            this.settings.set(
                                "statuschannel",
                        ...
                        .catch((e) => {
                            Util.error(e, "Logging", "sendMessage3")
                        })
                }
            }
            ...
            if (!data.players.sample && data.players.online > 0) {
                warningText =
                    ":warning: " + this.lang.STATUS_MESSAGE_TOOMANYPLAYERS
            ...
            if (warningText) {
                Util.getRecentMessage(this.channel, warningText)
                    .then((message) => {
                        if (message) return
                        Util.sendMessage(this.channel, warningText).catch(
                            (e) => {
                                Util.error(e, "Logging", "sendMessage4")
                            }
                        )
                    })
                    .catch((e) => {
                        Util.error(e, "Logging", "getRecentMessage1")
                    })
                return
            }
            ...
            if (!this.server.start) {
                current.forEach((player) => {
                    if (
                        old.filter(
                            (plr) =>
                                (plr.id && player.id && plr.id == player.id) ||
                                plr.name.clean == player.name.clean
                        ).length == 0
                    ) {
                        this.playerMessage(
                            player.name.clean,
                            this.lang.STATUS_PLAYER_JOINED
                        )
                    }
                })
        ...
        switch (Protocol.getErrorType(error)) {
            case "offline":
                if (
                    ((wasOnline || this.server.start) &&
                        (this.statusMessage.online == null ||
                            this.statusMessage.online == true)) ||
                    this.channelChanged
                ) {
                    errorText =
                        "<:red_circle_with_cross:818512512764084265> " +
                        this.lang[statusContents.offline]
                    callback = (message) => {
                        this.settings.set(
                            "statuschannel",
                            message.id,
                ...
                break
            case "notfound":
                errorText = ":warning: " + this.lang.SERVER_COULDNOTFIND
                break
            case "badport":
                errorText = ":warning: " + this.lang.SERVER_WRONGPORT
                break
            case "blocked":
                errorText = ":warning: " + this.lang.SERVER_BLOCKED
                break
            default:
                errorText = this.lang.STATUS_MESSAGE_ERROR
                Util.error(error, "Logging", "error")
        }

        if (errorText) {
            Util.getRecentMessage(this.channel, errorText)
                .then((message) => {
                    if (message) return
                    Util.sendMessage(this.channel, errorText)
                        .then(callback)
                        .catch((e) => {
                            Util.error(e, "Logging", "sendMessage5")
                        })
                })
                .catch((e) => {
                    Util.error(e, "Logging", "getRecentMessage2")
                })
        }
    }
    ...
    updateMessage(content, online) {
        ...
        promise
            ...
            .catch((e) => {
                Util.error(e, "Logging", "updateMessage")
            })
    }
    ...
    async _panel(data, imageMethod) {
        if (data.online) {
                ...
                },
                {
                    ...
                    value:
                        (data.bedrock
                            ? this.lang.BEDROCK
                            : data.modded
                            ? this.lang.MODS.format(data.mods.modList.length)
                            : this.lang.VANILLA) +
                        (data.plugins && data.plugins.length > 0
                            ? " " +
                              this.lang.PLUGINS.format(
                                  data.plugins.length
                              ).trim()
                            : ""),
                },
            ...
            const embed = new Discord.MessageEmbed()
                .setTitle(
                    `[${this.lang.AUTOMATED}] ` +
                        this.lang.COMMAND_STATUS_TITLE.format(
                            data.bedrock
                                ? `(${this.lang.BEDROCK})`
                                : `(${this.lang.JAVA})`
                        )
                ...
                .setDescription(
                    this.lang.COMMAND_STATUS_DESC.format(
                        this.server.ip +
                            ([19132, 25565].includes(this.server.port)
                                ? ""
                                : `:${this.server.port}`),
                        data.latency > 0
                            ? `\n${this.lang.SERVER_LATENCY.format(
                                  data.latency
                              )}`
                            : "",
                        data.players.online,
                        data.players.max,
                        data.gameMode
                            ? `\n${this.lang.SERVER_GAMEMODE.format(
                                  data.gameMode
                              )}`
                            : data.gameType
                            ? `\n${this.lang.SERVER_GAMETYPE.format(
                                  data.gameType
                              )}`
                            : "",
                        data.serverID
                            ? `\n${this.lang.SERVER_SEED.format(data.serverID)}`
                            : ""
                    )
                )
                .setColor(3092790)
            ...
            let faviconLink
            const faviconChannel = botGuild
                ? Util.getChannelById(
                      botGuild.channels,
                      LocalSettings.botserver.channels.favicons
                  )
                : null
            if (faviconChannel && data.favicon) {
                ...
                if (faviconLink) {
                    ...
                    content.embed.setThumbnail(faviconLink)
                ...
                favicon.onload = async () => {
                    ...
                    if (faviconLink) content.embed.setThumbnail(faviconLink)
        ...
        if (errorText) {
            const embed = new Discord.MessageEmbed()
                .setTitle(
                    `[${this.lang.AUTOMATED}] ` +
                        this.lang.COMMAND_STATUS_TITLE.format("").trim()
                )
                .setDescription(
                    this.lang.COMMAND_STATUS_FAIL_DESC.format(
                        this.server.ip,
                        this.server.port,
                        "\n\n" + errorText
                    )
    ...
    async panel(data) {
        this._panel(data, async (botGuild, content) => {
            ...
            if (motdLink) {
                ...
                const motdChannel = botGuild
                    ? Util.getChannelById(
                          botGuild.channels,
                          LocalSettings.botserver.channels.motds
                      )
                    : null
            ...
            if (motdLink) content.embed.setImage(motdLink)
        })
    }
    ...
    async panel_players(data) {
        this._panel(data, async (botGuild, content) => {
            ...
            if (playerListLink) {
                ...
                const playerListChannel = botGuild
                    ? Util.getChannelById(
                          botGuild.channels,
                          LocalSettings.botserver.channels.playerlists
                      )
                    : null
            ...
            if (playerListLink) content.embed.setImage(playerListLink)
    ...
    async update(startup = false) {
        ...
        if (!this.canSend) {
            ...
            let priorityChannel = Util.getPriorityChannel(this.guild, (chl) =>
                Util.hasPermissionsInChannel(this.guild.me, chl, [
                    "SEND_MESSAGES",
                ])
            )
            if (priorityChannel) {
                Util.sendError(
                    priorityChannel,
                    this.lang.STATUS_CHANNEL_NOPERMS
                )
            } else {
                console.log(
                    `[Logging] Did not have permissions to send messages in (${this.guild.id}) ${this.guild.name}'s status channel and could not find/send message in priority channel.`
                )
            }
            return
        }
        ...
        if (!this.statusMessage.message || this.statusMessage.message.deleted) {
            await new Promise(async (resolve) => {
                let messageId = await this.settings.get(
                    "statuschannel",
                    "MessageId"
                )
                Util.getMessageInChannel(this.channel, messageId)
                    .then((message) => {
                        this.statusMessage = {
                            message:
                    ...
                    .catch((e) => {
                        ...
                        statuses.forEach((status) => {
                            Util.getRecentMessageContaining(
                                this.channel,
                                this.lang[status.contentId]
                            )
                                .then((message) => {
                                    done++
                                    if (!message) return
        ...
        Protocol.getInfo(
            this.server.ip,
            this.server.port,
            this.server.queryPort
        )
            .then(async (data) => {
                displayMethod.call(this, data)
            })
            .catch((e) => {
                Util.error(e, "Logging", "getInfo")
            })
            .finally(() => {
